/*the code sets up an express router with two routes for handling booking requests and retrieving the most recent booking data. It uses the body-parser middleware to parse incoming request bodies and the cors middleware to enable cross-origin resource sharing. The Schema imported in the module defines the schema for the booking data that is stored in the database.*/
...
/*The first route /booking is a --POST-- request route that handles booking requests sent to the server. It first extracts data from the request body (movie, slot, and seats) and creates a new instance of the Schema with the extracted data. It then saves the new booking instance to the database and sends a success response with the booking data and a success message if the booking is successful. If the booking is not successful, it sends an error response with a null data and an error message.*/
...
/*The second route /booking is a --GET-- request route that retrieves the booking data from the database. It first finds the most recent booking data from the database using the find() method with the sort() method to sort the data by _id in descending order. If no booking data is found, it sends a response with a null data and a message. If booking data is found, it sends a success response with the booking data. */
